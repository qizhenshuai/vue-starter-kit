...
let android = ua.match(/(Android);?[\s/]+([\d.]+)?/) // 匹配 android
let ipad = ua.match(/(iPad).*OS\s([\d_]+)/) // 匹配 ipad
let ipod = ua.match(/(iPod)(.*OS\s([\d_]+))?/) // 匹配 ipod
let iphone = ua.match(/(iPhone\sOS)\s([\d_]+)/) // 匹配 iphone
